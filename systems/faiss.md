...
- ~~**GPU graph 索引**：§A.3 末尾明示 CAGRA 是 emerging direction；Faiss-GPU 当前只有 IVF 类。~~ **2026-05-09 ingest [ootomo-2023-cagra] 已 ingest**：[CAGRA](./cagra.md) 是 NVIDIA RAPIDS RAFT 的 GPU-native graph ANN——build 比 HNSW (CPU 64-core) 2.2-27× 快, large-batch search 33-77× 快。**Faiss 当前 release 仍未集成 RAFT/CAGRA**——这是 Meta vs NVIDIA 生态边界问题。Milvus 已通过 RAFT 集成（GPU_CAGRA 索引）。Faiss 集成 CAGRA 是 logical work，但论文 §A.3 后续未推进
