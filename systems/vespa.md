...
- **Vespa hybrid retrieval first-class production: BM25 + [SPLADE](../concepts/splade-sparse-retrieval.md) + dense rank-profile (NEW 2026-05-12 ingest)**: Vespa 是 wiki 内**唯一同时 first-class native** BM25 + SPLADE-style weightedset + dense tensor + 4-phase ranking (sparse-dense-rerank-global) **全 schema 一等公民** 的 system——rank-profile expression 可写: `0.4 × bm25(field) + 0.4 × dot_product(splade_field, query_splade) + 0.2 × closeness(vector_field)` 任意 ML-trained 或 constant 加权. SPLADE weightedset cell type 是 wiki 内**唯一原生支持 sparse neural retrieval output (BERT vocab 30K terms with learned weights)** 的 vendor. 与 Vespa BlockMaxWAND BM25 (Yahoo! 2003 web search lineage) + WAND/weakAnd retrieval operator 完美整合, **是 SPLADE production deployment 最自然候选**. **Open**: (a) Vespa SPLADE production case scale 公开数据? unknown; (b) BlockMaxWAND BM25 vs SPLADE 在 production hybrid (with cross-encoder rerank) 的 quality / cost head-to-head 实测? not published; (c) Vespa global-phase ONNX cross-encoder rerank 与 SPLADE first-stage 的 latency budget 配比? docs 不深入. 详见 [topics/sparse-dense-hybrid-retrieval.md](../topics/sparse-dense-hybrid-retrieval.md) 与 [concepts/splade-sparse-retrieval.md](../concepts/splade-sparse-retrieval.md).
- **Vespa "matryoshka tensor" cell type 命名来源 [MRL](../concepts/matryoshka-embedding.md) (NEW 2026-05-11 ingest)**: Vespa schema 中的 `tensor<float>(i{},x[N])` "matryoshka tensor" 命名直接来自 [kusupati-2022-matryoshka] NeurIPS 2022 paper——Vespa 是 wiki 内**第一个 (也是当前唯一) 把 Matryoshka Representation 作为 schema-level first-class cell type 的 vector DBMS**. 实际 production deployment 含义: (1) **单 schema 存 MRL 全维 + ranking-profile query 时选 prefix dim**——单 vector DB index 服务 multi-latency-tier (mobile 用 prefix 256, cloud 用全 1024) without re-embedding; (2) **4-phase ranking + MRL prefix 完美 fit Adaptive Retrieval pipeline** (per [topics/adaptive-retrieval-shortlist-rerank.md](../topics/adaptive-retrieval-shortlist-rerank.md)): retrieval 用 MRL 256-d prefix HNSW shortlist, first-phase / second-phase rerank 用 MRL 1024-d 全维, global-phase ONNX cross-encoder 最终 rerank; (3) **生产 case 直接对接** OpenAI text-embedding-3 / Cohere embed-v4 / Voyage-3 / Qwen3-VL-Embedding-8B 等 MRL-trained models. **Open**: Vespa matryoshka cell type 实际 production scale 公开数据 unknown; multi-prefix coexistence in single field (e.g., 单 doc 同时 256 + 512 + 1024 三个 prefix HNSW index) docs 不深入。
