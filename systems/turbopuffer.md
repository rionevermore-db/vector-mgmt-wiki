...
- **Turbopuffer hybrid (BM25 + vector) "application-layer fusion" 哲学 + [SPLADE](../concepts/splade-sparse-retrieval.md) sparse-side candidate (NEW 2026-05-12 ingest)**: Turbopuffer docs §hybrid 明示 "focused on first-stage retrieval, 2nd stage refine + rerank in your `search.py`/`search.ts`"——`multi_query` API 单 call 内 BM25 + vector 两个独立 query, **application-side RRF (reciprocal rank fusion) 或自定义 fusion**. 这是 wiki 内**最 minimal hybrid 哲学**: DBMS 仅服务 first-stage, application 做 fusion + rerank. **SPLADE production candidate**: Turbopuffer 客户可以 (a) namespace 存 dense vector + sparse SPLADE weights (via `rank_by` 不同 ranking function), (b) `multi_query` BM25 + SPLADE + dense 三个 first-stage retrievers, (c) application 端 RRF + cross-encoder rerank (Cohere / Voyage / mxbai 等 docs §hybrid 明示推荐). **优势**: full control + flexibility (vs Vespa rank-profile 内置 / Weaviate `hybrid()` API 黑盒); **劣势**: 1 extra hop (DBMS → app → DBMS) + application 复杂度. **Open**: (a) Turbopuffer 是否原生支持 SPLADE-encoded sparse vector 作 `rank_by` field type? docs `rank_by: ("body_splade", "BM25-style", splade_encode(query))` 未明示——可能技术上 work via custom sparse representation, docs 不深入; (b) Turbopuffer 在 multi_query 内 不同 ranking function 的 latency budget 分配? unknown. 详见 [topics/sparse-dense-hybrid-retrieval.md](../topics/sparse-dense-hybrid-retrieval.md).
