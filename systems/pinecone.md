...
- **Pinecone vs [Vespa](./vespa.md) SaaS-vs-OSS 哲学对比**：Pinecone 闭源 SaaS only + Inference + 黑盒 adaptive slab indexing；Vespa Apache-2.0 OSS + Vespa Cloud SaaS + 显式 4-phase ranking + first-class tensor framework。**Transparency 谱系两端**：Pinecone 极端不透明（"slab is fast"——不公开算法）；Vespa 极端透明（4-phase ranking、ONNX/XGBoost/LightGBM、application package 全开源）。**Heritage 不同**：Pinecone 2019 起步 vector-first SaaS（无 inverted index legacy）；Vespa 2003 Yahoo! 起步 web search（20 年 inverted index + ranking pipeline 沉淀）。**Ranking 能力**：Vespa 4-phase ranking 适合复杂 LTR pipeline（reranker + tensor compute）；Pinecone 仅有简单 score 返回，复杂 ranking 推到 application 层。**Deployment**：Pinecone 仅 SaaS；Vespa OSS self-host + Vespa Cloud + on-prem。详见 [systems/vespa.md "对比同类 OSS vector DBMS"](./vespa.md)。
- **Pinecone vs [Turbopuffer](./turbopuffer.md) closed SaaS 同代对比**——transparency 反差**两个极端**：Pinecone "managed simplicity" 哲学下**完全不公开算法**（"slab adaptive index 是 fast"——HNSW？IVF+PQ？SPANN-style？外部无法验证）；Turbopuffer **同样闭源但完全 transparent design**——明示 SPFresh + LSM tree on object storage + WAL on object storage + 3-tier cache 全 stack。**Storage 哲学不同**：Pinecone 不暴露（推测 NVMe + S3 backup）；Turbopuffer **object storage as primary 唯一 stateful 依赖**——所有 compute stateless，cost reduction 来源是"S3 比 EBS 便宜 10×"。**Scale claim 对比**：Pinecone "billion-scale capability" 但具体数字不公开；Turbopuffer **3.5T+ docs / 100M+ namespaces / 100B+ vectors queryable 全公开**——transparency 直接体现在公开数字上。**Multi-tenancy 默认级别不同**：Pinecone namespace within index (有限 hierarchy)；Turbopuffer namespace = S3 prefix (100M+ 一等公民 architectural primitive)。**OSS prospect**：Pinecone tradeoffs 明示"永远闭源"姿态；Turbopuffer tradeoffs 措辞"for the current phase commercial-only... offer BYOC"——留 OSS 余地。详见 [systems/turbopuffer.md "Open Questions"](./turbopuffer.md)。
