...
- ~~**更新与删除**：与 [NSG](../concepts/nsg.md) 一样不支持增量；graph-based 路线（DiskANN/Vamana）的 in-place update 仍是开放问题~~ **2026-05-09 ingest [singh-2021-freshdiskann] 已解**：[FreshDiskANN](./freshdiskann.md) 用 [FreshVamana](../concepts/freshvamana.md) (α=1.2) 给出 graph-path 的 streaming 解——sustaining 1800+1800 inserts/deletes/sec on 800M SIFT @ 95+% recall；StreamingMerge **5.25× faster than DiskANN full rebuild** (15832s vs 83140s)。**关键 insight**：DiskANN 的 Vamana α 参数从"性能调节钮"升级为 fresh-ANNS 必要条件；α=1（HNSW/NSG default）失败，α>1 work。Cluster-based 路径有 [SPFresh](./spfresh.md) (LIRE)，graph-based 路径有 [FreshDiskANN](./freshdiskann.md) (FreshVamana)——两条 in-place 路径并存，内存预算 4 GB vs 128 GB 决定选择。详见 [topics/in-place-vs-out-of-place-updates.md](../topics/in-place-vs-out-of-place-updates.md)
