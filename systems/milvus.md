...
| **Quantization (RaBitQ)** | **IVF_RABITQ** | NEW，[RaBitQ](../concepts/rabitq.md) unbiased + sharp error bound;[per sources/docs/milvus/site/en/userGuide/indexes/floating-vector/ivf-rabitq.md] |
...
- **2.6.x 实测 benchmark**：[SIGMOD 2021 论文](../benchmarks/milvus-vs-prior-sift10m-deep10m.md) 是 1.x；[VLDB 2022 Manu 论文](../benchmarks/manu-vs-elasticsearch-vearch-vald-vespa.md) 实测到 100M scale；**v2.6.x cloud-native 进一步演化后的逐版本实测数字仍是 vendor-published**——但跨厂商横测方法论现已 ingest：见 [benchmarks/vectordbbench.md](../benchmarks/vectordbbench.md)（系统级 load duration / QP$ / 持续插入下 QPS，含 100M LAION XLarge case + Cohere-10M streaming insertion-under-load）。**注意 VectorDBBench 由 Zilliz（Milvus 母公司）自维护**,绝对数值有自测偏向且随 leaderboard 滚动,引用须交叉验证 + 标 "as of"（详见 [topics/ann-benchmarking-methodology.md](../topics/ann-benchmarking-methodology.md) "benchmarks lie" 段）。Zilliz 团队亦是 [big-ann-benchmarks](../benchmarks/big-ann-benchmarks.md) NeurIPS 2021 Track 2（SSD）冠军 BBANN（= [Manu §4.4 SSD-aware indexing](../concepts/manu-ssd-hierarchical-kmeans.md)）
...
- ~~**Milvus quantizer 升级到 [RaBitQ](../concepts/rabitq.md)**：社区 PR 是 logical next step~~ **2026-05-21 lint 已解**：Milvus v2.6.x **已内置 IVF_RABITQ 索引**[per sources/docs/milvus/site/en/userGuide/indexes/floating-vector/ivf-rabitq.md]——RaBitQ 不再是"next step"而是已发布 index type（见上方 v2.6.x 索引族表 + [concepts/rabitq.md "Production 采用"](../concepts/rabitq.md)）。**仍开放**：(a) IVF_RABITQ 与 Milvus partition-based attribute filter 联合时 5 strategy 中 E 是否仍最优、(b) IVF_RABITQ vs IVF_PQ 在 Milvus 上的实测 recall/QPS（docs 未给数）
- **Milvus disk graph 升级到 [Starling](./starling.md)**：[per wang-2024-starling §8] Starling 是 Zilliz/Milvus 团队 SIGMOD 2024 论文（Charles Xie + Rentong Guo 都是 Manu/Milvus 论文作者）；明示 future work "integrate Starling into Milvus for distributed optimization"——v2.6.x docs 当前 DISKANN 索引仍是原版 DiskANN，Starling 是**下一代候选**。Starling 在 Milvus segment 配置（2GB RAM + 10GB disk）下 ANNS 2× 快于 DiskANN，RS 43.9× 快——直接对应 Milvus segment 模型设计目标。集成时机未公开
- **Milvus vs [Qdrant](./qdrant.md) 架构哲学对比**：Milvus（Go + cloud-native disaggregated + 多 index_type 生态）vs Qdrant（Rust 单 binary + HNSW only + 简洁路径）。两者代表 OSS vector DBMS 的两条工程路径：Milvus "tool box for all workloads" vs Qdrant "do HNSW exceptionally well"。Milvus 多 index (HNSW/IVF*/DISKANN/CAGRA/SPARSE) 适合多 workload 集群部署；Qdrant 仅 HNSW + filter-aware extra edges 适合 simple production + Rust performance。详见 [systems/qdrant.md "与 Milvus 的对比"](./qdrant.md)。
- **Milvus vs [Weaviate](./weaviate.md) 定位对比**：Milvus "vector data management system"（vector-first，但支持多 workload + index 多样性） vs Weaviate "AI-native primary database"（vector + objects + 完整 inverted index 套件 + agent stack）。两者**生态定位不同**：Milvus 走"多 index_type tool box"；Weaviate 走"agent stack 集成 + primary DB"。Milvus 300+ enterprise (Salesforce/NVIDIA/PayPal/eBay)；Weaviate Cloud + Query Agent / Engram stack 生态。详见 [systems/weaviate.md "与 Milvus 的对比"](./weaviate.md)。
- **Milvus vs [Vespa](./vespa.md) lineage 对比**：Milvus = "vector-first DBMS from Day 1"（2019 起就是 vector-native，cloud-native disaggregated 重写）vs Vespa = "web search engine (Yahoo! 2003) + vector retrofit"（先有 inverted index + ranking pipeline 20 年，2017 OSS，后加 HNSW + tensor）。Vespa 的工程沉淀：4-phase ranking（retrieval → first-phase → second-phase → global-phase ONNX）、stateless container / stateful content 分离、first-class tensor framework (dense/sparse/mixed, ONNX/XGBoost/LightGBM native)、Streaming Search（no-index brute-force per-user, 45 B/doc, billions/node）——这些**都是 web search heritage**，Milvus 没有等价物。Milvus 优势：纯 vector workload 的 index 多样性（HNSW/IVF*/DISKANN/CAGRA/SPARSE 任选）+ cloud-native segment 模型，Vespa 索引选择更窄（HNSW + Streaming）但 ranking pipeline 更成熟。详见 [systems/vespa.md "对比同类 OSS vector DBMS"](./vespa.md)。
- **Milvus vs [Turbopuffer](./turbopuffer.md) OSS-vs-closed + storage 哲学对比**：Milvus = OSS Go DBMS + cloud-native segment 模型（NVMe + S3 backup, **SSD 是 hot path**）；Turbopuffer = closed Rust SaaS + **object storage as primary storage**（S3 是 source of truth, NVMe 仅 cache）——**storage 主从关系完全倒置**。**ANN 算法选择空间**：Milvus 多 index_type tool box (HNSW/IVF*/DISKANN/CAGRA/SPARSE)；Turbopuffer **only SPFresh**——SPFresh paper (xu-2023-spfresh, SOSP 2023) 的首个 OSS-known production deployment（Milvus 团队和 Bing/SPANN 团队各自实现 SPFresh 路径不同：Milvus 选 DISKANN/CAGRA 等 graph 路径，Turbopuffer 选 SPFresh centroid 路径）。**Scale claim 公开度**：Milvus public benchmark 仅到 100M (Manu paper)；Turbopuffer **3.5T+ docs / 100B+ vectors queryable / 100M+ namespaces 全部 docs 公开**——transparency 优势。**Multi-tenancy 设计层**：Milvus partition within collection；Turbopuffer namespace = S3 prefix (100M+ 一等公民，更 aggressive)。**部署模型**：Milvus cloud-native disaggregated 但需 ops complexity（streaming/query/data node 协调 + etcd + Pulsar/Kafka）；Turbopuffer "0 ops" SaaS——object storage 是唯一 stateful 依赖。详见 [systems/turbopuffer.md "对比"](./turbopuffer.md)。
